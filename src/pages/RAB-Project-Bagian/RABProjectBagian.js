...
const RABProjectBagian = () => {
  ...
  useEffect(() => {
    ...
    console.log(
      "Info: RABPBs(dot)selectedOption is changed to " + RABPBs.selectedOption
    );
    ...
    if (RABPBs.selectedOption === "FETCH") {
          ...
          response.RABProjectBagian &&
            (tableData = response.RABProjectBagian.map((rabpb, idx) => {
              const data = {
                ...
                idTtd: rabpb.ID_TTD,
                keteranganJudulRekap: rabpb.KETERANGAN_JUDUL_REKAP,
                ...
                keteranganBagBawahRab: rabpb.KETERANGAN_BAG_BAWAH_RAB,
                key: j.toString(),
                // ID_WILAYAH
              };
              j++;
      ...
      if (RABPBs.selectedIndex > -1) {
        fetch(hostname + "/project/update-rab-project-bagian?TAHUN=" + tahun, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          .then((response) => {
            if (response.ok) {
              var newDatas = JSON.parse(JSON.stringify(RABPBs.data));
